...
function uploader() {
	 ...
	 */
	function __convertToDec(deg, min, sec) {
		var dec;
		if (deg < 0) {
	...
	function parseExif(filePath, callback) {
		...
		new exif.ExifImage({ image: filePath}, function (err, data) {
				...
				if (data.gps) {
					location['latitude'] = __convertToDec(data.gps.GPSLatitude[0], data.gps.GPSLatitude[1], data.gps.GPSLatitude[2]);
					location['longitude'] = __convertToDec(data.gps.GPSLongitude[0], data.gps.GPSLongitude[1], data.gps.GPSLongitude[2]);
